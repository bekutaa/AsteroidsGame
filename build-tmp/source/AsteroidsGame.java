...
public void draw() 
{
  ...
  if(isShield)
  {
    ...
    sharkKnight.setEnergy(sharkKnight.getEnergy()--);
  }
  sharkKnight.show();
...
public void keyPressed() //Spaceship movement
{
  ...
  if(sharkKnight.getCrash() == false)
  {
    ...
    if(key == 'w' || (key == CODED && keyCode == UP)) { sharkKnight.accelerate(0.15f); }
    if(key == 's' || (key == CODED && keyCode == DOWN)) { sharkKnight.accelerate(-0.15f); }
...
class Alien extends Floater implements Space
{
  ...
  public void move(SpaceShip theShip)
  {
    ...
    {
      accelerate(Math.random()*0.2f);
    }
    else if(dist(theShip.getX(),theShip.getY(),(float)myCenterX,(float)myCenterY) > 50)
    {
      accelerate(Math.random()*0.3f);
    }
    else
    {
      accelerate(Math.random()*0.5f);
    }
  ...
  public void show()
  {
    ...
    arc((float)myCenterX,(float)myCenterY,24,20,(float)((0.4f/3)*Math.PI),(float)((2.6f/3)*Math.PI));
