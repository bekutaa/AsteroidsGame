...
public void draw() 
{
  ...
  if( ((SpaceShip)ichi[ichi.length-1]).getCrash() == false)
  {
    fill(255,0,0);
    textSize(12);
    text("Time survived so far: " + ((int)((timeSurvived/60)*10))/10.0f + "s",435,585);
...
public void keyPressed()
{
  if(((SpaceShip)ichi[ichi.length-1]).getCrash() == false)
  {
    ...
    if(key == 'w' || (key == CODED && keyCode == UP)) { ((SpaceShip)ichi[ichi.length-1]).accelerate(0.1f); }
    if(key == 's' || (key == CODED && keyCode == DOWN)) { ((SpaceShip)ichi[ichi.length-1]).accelerate(-0.1f); }
